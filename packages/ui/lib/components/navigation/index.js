import React, { useMemo } from 'react';
import Taro from '@tarojs/taro';
import { View, Image, Block } from '@tarojs/components';
import { useNavigationBarInfo } from './../../utils/hooks';
import Router from './../../utils/router';
const icon_back = `data:image/svg+xml;base64,`;
const icon_home = `data:image/svg+xml;base64,`;
export const XNavigation = (props) => {
    const { title = '页面标题', controlled = false, backVisible = true, homeVisible = true, onBackClick, onHomeClick } = props;
    const { statusBarHeight, navigationBarHeight, navigationContentHeight, menuButtonHeight, navigationPaddding, menuButtonWidth, } = useNavigationBarInfo();
    const handleBack = () => {
        if (!controlled) {
            Router.navigateBack();
        }
        else {
            if (onBackClick) {
                onBackClick();
            }
        }
    };
    const handleBackToHome = () => {
        if (!controlled) {
            Router.backToHome();
        }
        else {
            if (onHomeClick) {
                onHomeClick();
            }
        }
    };
    const visibleIconNum = useMemo(() => {
        const backNum = backVisible ? 1 : 0;
        const homeNum = homeVisible ? 1 : 0;
        return backNum + homeNum;
    }, []);
    const iconGroupStyle = useMemo(() => {
        if (visibleIconNum === 1) {
            return {
                width: `${menuButtonHeight}px`,
                borderRadius: `${menuButtonHeight / 2}px`
            };
        }
        else if (visibleIconNum === 2) {
            return {
                width: Taro.pxTransform(160),
                borderRadius: `${menuButtonHeight / 2}px`
            };
        }
    }, [visibleIconNum]);
    return (React.createElement(View, { className: 'nav-header', style: {
            height: navigationBarHeight,
            padding: `0 ${navigationPaddding}px`,
        } },
        React.createElement(View, { className: 'nav-header-status-placeholder', style: {
                height: statusBarHeight,
            } }),
        React.createElement(View, { className: 'nav-header-content', style: {
                height: `${navigationContentHeight}px`,
            } },
            React.createElement(View, { className: 'nav-header-content-left', style: {
                    width: menuButtonWidth,
                } }, visibleIconNum > 0 ?
                React.createElement(View, { className: `nav-header-content-left-icon-group ${visibleIconNum === 1 ? 'single' : ''}`, style: Object.assign(Object.assign({}, iconGroupStyle), { height: `${menuButtonHeight}px` }) },
                    backVisible &&
                        React.createElement(Image, { src: icon_back, className: 'nav-header-content-left-icon', onClick: handleBack }),
                    homeVisible && React.createElement(Block, null,
                        backVisible &&
                            React.createElement(View, { className: 'nav-header-content-left-icon-divider' }),
                        React.createElement(Image, { src: icon_home, className: 'nav-header-content-left-icon', onClick: handleBackToHome })))
                : null),
            React.createElement(View, { className: 'nav-header-content-center' }, title || '页面标题'),
            React.createElement(View, { className: 'nav-header-content-right', style: {
                    width: menuButtonWidth,
                } }))));
};
export default XNavigation;
//# sourceMappingURL=index.js.map